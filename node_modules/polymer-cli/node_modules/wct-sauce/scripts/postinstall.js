...
if (!(travis.isTravisSauceConnectRunning() ||
      process.env.SKIP_WCT_SAUCE_POSTINSTALL_DOWNLOAD)) {
    ...
    sauceConnectLauncher.download(
        ...
        function(error) {
          if (error) {
            ...
            console.log(
                'sauce-connect-launcher will attempt to re-download next time it is run.');
            // We explicitly do not fail the install process if this happens;
            // the user can still recover, unless their permissions are
            // completely screwey.
          }
        });
  });
